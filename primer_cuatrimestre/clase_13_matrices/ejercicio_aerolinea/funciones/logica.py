...
def validar_ingreso_codigo_vuelo(codigo):
    ingreso_valido = False

    while not ingreso_valido:
        for i in range(len(codigo)):
            while len(codigo) < 5 or len(codigo) > 5:
                print("\nERROR...")
...
def buscar_vuelo_por_codigo(vuelos):
...
def buscar_vuelo_mas_caro(vuelos):
...
def buscar_vuelo_mas_barato(vuelos):
...
def mostrar_vuelos_mayores_a_valor(vuelos):
